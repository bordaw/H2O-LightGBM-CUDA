...
std::chrono::duration<double, std::milli> copy_subset_time;
std::chrono::duration<double, std::milli> tree_time;
std::chrono::duration<double, std::milli> total_train_time; 
std::chrono::duration<double, std::milli> bagging_gh_time; 
#endif // TIMETAG
...
GBDT::~GBDT() {
  ...
  if (config_ != nullptr) {
  #ifdef TIMETAG
  Log::Info("LightGBM-CUDA::total train time %f", total_train_time * 1e-3);
  Log::Info("LightGBM-CUDA::boosting costs %f", boosting_time * 1e-3);
  Log::Info("LightGBM-CUDA::train_score costs %f", train_score_time * 1e-3);
  Log::Info("LightGBM-CUDA::out_of_bag_score costs %f", out_of_bag_score_time * 1e-3);
  Log::Info("LightGBM-CUDA::valid_score costs %f", valid_score_time * 1e-3);
  Log::Info("LightGBM-CUDA::metric costs %f", metric_time * 1e-3);
  Log::Info("LightGBM-CUDA::copy subset costs %f", copy_subset_time * 1e-3);
  Log::Info("LightGBM-CUDA::bagging costs %f", bagging_time * 1e-3);
  Log::Info("LightGBM-CUDA::tree costs %f", tree_time * 1e-3);
  Log::Info("LightGBM-CUDA::bagging costs %f", bagging_gh_time * 1e-3);
  #endif
  }
}
  ...
  if ((bag_data_cnt_ < num_data_ && iter % config_->bagging_freq == 0)
    ...
    if (!is_use_subset_) {
     ...
     if (resized && (config_->device_type == std::string("cuda"))) {
      ...
      #ifdef TIMETAG // LGBM_CUDA
      auto start_copy_subset_time = std::chrono::steady_clock::now();
      #endif

      tmp_subset_->CopySubset(train_data_, bag_data_indices_.data(), bag_data_cnt_, false);

      #ifdef TIMETAG  // LGBM_CUDA
      copy_subset_time += std::chrono::steady_clock::now() - start_copy_subset_time;
      #endif
...
#ifdef TIMETAG //LGBM_CUDA
    total_train_time = end_time - start_time; // LGBM_CUDA
#endif
 ...
 if ((config_->device_type == std::string("cuda")) && (iter_ == 0)) {
  ...
  for (int cur_tree_id = 0; cur_tree_id < num_tree_per_iteration_; ++cur_tree_id) {
    ...
    if (class_need_train_[cur_tree_id] && train_data_->num_features() > 0) {
     ...
     #ifdef TIMETAG
      bagging_gh_time += std::chrono::steady_clock::now() - start_time;
      #endif
