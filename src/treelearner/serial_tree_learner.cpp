...
#ifdef TIMETAG
  Log::Info("LightGBM-CUDA::init_train costs %f", SerialTreeLearner::init_train_time_ * 1e-3);
  Log::Info("LightGBM-CUDA::init_split costs %f", init_split_time_ * 1e-3);
  Log::Info("LightGBM-CUDA::hist_build costs %f", hist_time_ * 1e-3);
  Log::Info("LightGBM-CUDA::find_split costs %f", find_split_time_ * 1e-3);
  Log::Info("LightGBM-CUDA::split costs %f", split_time_ * 1e-3);
  Log::Info("LightGBM-CUDA::ordered_bin costs %f", ordered_bin_time * 1e-3);
#endif
...
Tree* SerialTreeLearner::Train(const score_t* gradients, const score_t *hessians, bool is_constant_hessian, Json& forced_split_json) {
  ...
  #ifdef TIMETAG
  init_train_time_ += std::chrono::steady_clock::now() - start_time;
  #endif
  ...
  for (int split = init_splits; split < config_->num_leaves - 1; ++split) {
    ...
    if (!aborted_last_force_split && BeforeFindBestSplit(tree.get(), left_leaf, right_leaf)) {
      #ifdef TIMETAG
      init_split_time_ += std::chrono::steady_clock::now() - start_time;
      #endif
    ...
    #ifdef TIMETAG
    split_time_ += std::chrono::steady_clock::now() - start_time;
    #endif
...
void SerialTreeLearner::FindBestSplitsFromHistograms(const std::vector<int8_t>& is_feature_used, bool use_subtract) {
  ...
  #ifdef TIMETAG
  find_split_time_ += std::chrono::steady_clock::now() - start_time;
  #endif
}
