...
CUDATreeLearner::~CUDATreeLearner() {
  ...
  for(int device_id = 0; device_id < num_gpu_; ++device_id) {
    Log::Info("LightGBM-CUDA Device[%d] kernel costs %f", device_id, kernel_time_[device_id] * 1e-3);
    Log::Info("LightGBM-CUDA Device[%d] kernel_input_memcopy costs %f", device_id, kernel_input_wait_time_[device_id] * 1e-3);
    Log::Info("LightGBM-CUDA Device[%d] kernel_output_memcopy costs %f", device_id, kernel_output_wait_time_[device_id] * 1e-3);
  }
  #endif
}
