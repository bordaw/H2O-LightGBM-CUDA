...
// LGBM_CUDA
#ifdef TIMETAG
  Log::Info("LightGBM-CUDA init train time %f", init_train_time * 1e-3);
  Log::Info("LightGBM-CUDA load data time %f", load_data_time * 1e-3);
  Log::Info("LightGBM-CUDA train time %f", train_time * 1e-3);
  Log::Info("LightGBM-CUDA save model time %f", save_model_time * 1e-3);
#endif
